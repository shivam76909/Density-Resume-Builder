"""
update_footers_v2.py
Handles BOTH <footer>...</footer> AND simple <div class="bottom-bar">...</div> replacements.
Also injects footer CSS if needed and updates grid columns.
"""
...
# Full footer CSS block to inject (before </style>)
FOOTER_CSS = '''
        /* ===== FOOTER ===== */
        .footer {
            background: #0f172a;
        ...
        @media (max-width: 1200px) { .footer-grid { grid-template-columns: 1fr 1fr; gap: 40px; } }
...
def process_file(fpath):
    ...
    # 1) Replace bottom-bar div pattern (simple old footer)
    bottom_bar_pattern = re.compile(
        r'(\s*)<!--\s*BOTTOM BAR\s*-->[\s\S]*?</div>(?=\s*\n*\s*</body>)',
        re.IGNORECASE
    )
    if bottom_bar_pattern.search(content):
        content = bottom_bar_pattern.sub(NEW_FOOTER_HTML, content, count=1)
    else:
        # Try without comment — look for <div class="bottom-bar">
        bottom_bar_pattern2 = re.compile(
            r'\s*<div class=["\']bottom-bar["\'][\s\S]*?</div>\s*(?=\n?\s*</body>)',
            re.IGNORECASE
        )
        if bottom_bar_pattern2.search(content):
            content = bottom_bar_pattern2.sub(NEW_FOOTER_HTML, content, count=1)
        else:
            # No replacement possible
            return "SKIPPED (pattern not found)"

    # 2) Inject footer CSS before </style> if footer CSS not present
    if "footer-grid" not in content:
        content = content.replace("</style>", FOOTER_CSS + "\n    </style>", 1)

    # 3) Update 4-col to 5-col if present
